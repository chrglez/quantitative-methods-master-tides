...
The repo does **not** ship CSV/XLSX files — they are gitignored and
fetched on demand from the source APIs. After cloning, run this once
...
The script writes Eurostat and ISTAC files into `datasets/raw/` and a
`MANIFEST.md` recording filenames, source URLs and download date.

::: {.callout-warning}
**If `install.packages("eurostat")` fails** (CRAN occasionally archives
...
Fallback if R-universe is unreachable:
`remotes::install_github("rOpenGov/eurostat")` (slower, compiles).
:::
